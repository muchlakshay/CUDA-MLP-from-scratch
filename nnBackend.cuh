...
void testPrint(){
    float arr[4] {1,2,3,4};
    float arr2[4] {1,3,2,4};
    float arr3[4];
    float* d_arr;
    float* d_arr2;
    float* d_arr3;
    cudaMalloc(&d_arr, 4*sizeof(float));
    cudaMalloc(&d_arr2, 4*sizeof(float));
    cudaMalloc(&d_arr3, 4*sizeof(float));

    cudaMemcpy(d_arr, arr, 4*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_arr2, arr2, 4*sizeof(float), cudaMemcpyHostToDevice);
    debugPrint<<<1,1>>>(d_arr, 2, 2);
    cudaDeviceSynchronize();
    debugPrint<<<1,1>>>(d_arr2, 2, 2);
    cudaDeviceSynchronize();
    ...
    // Launch the kernel
    MatMul<<<gridDim, blockDim>>>(d_arr, d_arr2, d_arr3, M, N, K);

    cudaMemcpy(arr3, d_arr3, 4*sizeof(float), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    cout<<"\nMatmul: \n";
    for (int i {}; i<4; ++i){
        cout<<arr3[i]<<" ";
